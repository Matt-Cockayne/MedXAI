"""
Example: Evaluate explainability methods on ChestMNIST dataset (MedMNIST).

This script demonstrates:
- Loading ChestMNIST from MedMNIST (14 thoracic diseases)
- Generating explanations for multi-class chest X-ray classification
- Quantitative evaluation
- Visualizing results

ChestMNIST: Multi-label classification of 14 common thoracic diseases
"""
...
def main():
    print("=" * 70)
    print("MedMNIST ChestMNIST Explainability Evaluation")
    print("=" * 70)
    ...
    print(f"   Examples: {', '.join(class_names[:4])}...")
    ...
    # ChestMNIST is multi-label, so labels is an array
    # Find the first positive label (if any) or use the first label
    import numpy as np
    if isinstance(labels, np.ndarray):
        ...
        if len(positive_labels) > 0:
            ...
            label = 0  # Default to first class if no positive labels
    else:
        label = int(labels)
    ...
    print("\nChestMNIST contains 14 thoracic disease classes from NIH ChestX-ray14")
